...
const VERCEL_URL = 'https://kyatwise.vercel.app';
...
export class InvitationService {
  ...
  sendInvitationEmail(
    ...
    const subject = isMyanmar
      ? `${groupName} သို့ Kyat Wise တွင် ပါဝင်ရန် သင့်အား ဖိတ်ခေါ်ပါသည်။`
      : `You're invited to join ${groupName} on Kyat Wise`;

    const texts = {
      title:    isMyanmar ? 'သင့်ကို ဖိတ်ခေါ်ထားပါတယ်' : "You're Invited!",
      greeting: isMyanmar ? 'မင်္ဂလာပါ,' : 'Hi,',
      body1: isMyanmar
        ? `${inviterName} မှ သင့်အား Kyat Wise ရှိ သူတို့၏အဖွဲ့ <strong>"${groupName}"</strong> သို့ ဖိတ်ခေါ်ထားပါသည်။`
        : `${inviterName} has invited you to join their group, <strong>"${groupName}"</strong>, on Kyat Wise.`,
      body2: isMyanmar
    ...
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #333; line-height: 1.6; max-width: 600px; margin: 20px auto; border: 1px solid #ddd; border-radius: 12px; overflow: hidden;">
      <div style="background-color: ${PrimaryBgColor}; color: ${PrimaryTxtColor}; padding: 20px; text-align: center;">
        <h1 style="margin: 0; font-size: 28px;">Kyat Wise</h1>
      </div>
      ...
      <div style="background-color: #f7f7f7; color: #888; padding: 15px; text-align: center; font-size: 12px;">
        &copy; ${new Date().getFullYear()} Kyat Wise. All Rights Reserved.
      </div>
    </div>
    `;
